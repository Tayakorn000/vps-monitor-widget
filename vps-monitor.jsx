...
const saveConfig = (e) => {
  ...
  const pass = document.getElementById('vps-input-pass').value
  if (!host || !user || !pass) return

  const config = JSON.stringify({ host, user, pass })

  // write config via osascript
  const cmd = `osascript -e 'do shell script "echo ${btoa(config)} | base64 -d > $HOME/.vps-widget-config"'`
  run(cmd)
  ...
  return (
    <div id="vps-widget" style={{ ...styles.container, left: pos.x, top: pos.y }} onMouseDown={onMouseDown}>
      ...
      <div style={styles.header}>
        ...
        <button id="vps-settings-btn" style={styles.gearBtn} onClick={toggleConfig} title="Settings">⚙</button>
      </div>
      ...
      {!showConfig && (
        isError ? (
          <div style={styles.offline}>
            {configSaved ? '⚠ Offline' : '⚙ Click ⚙ to configure'}
          </div>
        ) : (
          <div>
...
const styles = {
  ...
  dotGreen: {
    ...
    background: '#4cff91', boxShadow: '0 0 6px #4cff91', display: 'inline-block', flexShrink: 0,
  },
  dotRed: {
    width: '8px', height: '8px', borderRadius: '50%',
    background: '#ff4d4d', display: 'inline-block', flexShrink: 0,
  },
  title: {
    fontSize: '13px', fontWeight: '600', color: 'rgba(255,255,255,0.85)', flex: 1,
  },
  gearBtn: {
  ...
  configTitle: {
    fontSize: '11px', color: 'rgba(255,255,255,0.5)', marginBottom: '8px', letterSpacing: '0.5px',
  },
  input: {
  ...
  saveBtn: {
    ...
    borderRadius: '8px', color: '#4cff91', fontSize: '12px',
    fontWeight: '600', cursor: 'pointer',
  },
  row: {
    display: 'flex', alignItems: 'center', marginBottom: '10px', gap: '8px',
  },
  label: {
    fontSize: '12px', color: 'rgba(255,255,255,0.5)', width: '32px', flexShrink: 0,
  },
  barBg: {
  ...
  value: {
    fontSize: '11px', width: '90px', textAlign: 'right', flexShrink: 0,
  },
  offline: {
    fontSize: '13px', color: 'rgba(255,255,255,0.4)', textAlign: 'center', padding: '8px 0',
  },
}
